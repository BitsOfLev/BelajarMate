...
<div class="p-6 space-y-4">
    ...
    <div class="flex justify-between items-center flex-wrap gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Education Levels</h2>
            <p class="text-sm text-gray-500 mt-1">Manage and review education levels</p>
        </div>
        <div>
            <a href="{{ route('admin.data-management.education-level.create') }}" 
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mt-4">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Education Level</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">#{{ $level->edulvlID }}</td>
                        <td class="px-4 py-3 text-sm">{{ $level->edulvlType }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-2">
                                <!-- Edit Button -->
                                <a href="{{ route('admin.data-management.education-level.edit', $level->edulvlID) }}" 
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('admin.data-management.education-level.destroy', $level->edulvlID) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this education level? This action cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
        ...
        <div class="bg-white px-4 py-3 rounded-lg shadow-sm border border-gray-200 mt-2">
